  ...
  render() {
      ...
      <li key={`section-${story_idx}`} className={this.classes.listSection}>
        <ul className={this.classes.ul}>
          <ListSubheader color={"primary"}>
            <ListItem button onClick={this.click.bind(this, story.timestamp)}>
              <ListItemIcon>
                <VideocamIcon />
              </ListItemIcon>
              <ListItemText
          ...
          {story.outline.map((item, item_idx) => (
            ...
            >
              <ListItem
                key={`item-${story_idx}-${item_idx}`}
                button
                onClick={this.click.bind(this, item.timestamp)}
              >
                <ListItemAvatar>
    ...
    let searchResults = this.state.search_data.map((elem, idx) => (
      <li key={`section-${idx}`} className={this.classes.listSection}>
        <ListItem button onClick={this.click.bind(this, elem.start)}>
          <ListItemAvatar>
            <Avatar>
              <PlayArrowIcon />
            </Avatar>
            ...
            <Card>
              <CardBody>
                <Player
                  ...
                  src={
                    this.props.videoUrl == ""
                      ? require("assets/video/03_linear-algebra-review.mp4")
                      : this.props.videoUrl
                  }
                />
              </CardBody>
              ...
              <CustomInput
                ...
                inputProps={{
                  ...
                  onChange: (event) => {
                    ...
                    this.setState({
                      search_data: getSearchResult(
                        event.target.value,
                        this.state.seg_result
                      ),
                    });
                  },
                  startAdornment: (
